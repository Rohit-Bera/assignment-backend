...
const askForPaymentService = async ({
  ...
  try {
    ...
    if (newPayment) {
      const percent = assignmentCost * 0.1;
...
const getBalanceWalletService = async ({ _id }) => {
  try {
    const myBalance = await Wallet.findOne({ user: _id });
...
const withdrawFromWallet = async ({ _id, withdrawlAmt }) => {
  try {
    const myWallet = await Wallet.findOne({ user: _id });
...
// client
const payAmountService = async ({ clientId, _id, paymentInfo }) => {
  try {
    ...
    const { order, assignmentCost } = findPayment;
    ...
    // ----------------- commission
    const percent = assignmentCost * 0.1;
    const finalAmount = assignmentCost - percent;
    ...
    // ----------------- wallet
    if (order) {
      const findWallet = await Wallet.findOne({ user: order.user });
